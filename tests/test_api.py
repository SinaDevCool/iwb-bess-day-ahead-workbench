def test_health_and_configuration(client):
...
def test_forecast_rejects_unsupported_or_invalid_intervals_and_dates(client):
...
def test_product_duration_changes_dispatch_granularity_and_orders(client):
    hourly = client.post("/api/simulations", json={"market": {"product_minutes": 60}}).json()
    quarter_hourly = client.post(
        "/api/simulations", json={"market": {"product_minutes": 15}}
    ).json()
...
def test_saved_runs_have_decision_oriented_names_and_can_be_renamed(client):
    baseline = client.post("/api/simulations", json={}).json()
    constrained = client.post(
        "/api/simulations", json={"battery": {"unavailable_intervals": [6, 7]}}
    ).json()
    ...
    assert (
        client.get(f"/api/simulations/{constrained['simulation_id']}").json()["display_name"]
        == "Morning maintenance"
    )
    catalogue = client.get("/api/simulation-runs", params={"limit": 100}).json()["items"]
    assert (
        next(item for item in catalogue if item["simulation_id"] == constrained["simulation_id"])[
            "display_name"
        ]
        == "Morning maintenance"
    )
...
def test_saved_run_name_validation_is_bounded(client):
...
def test_simulation_approval_and_audit(client):
...
def test_trader_can_adjust_and_exclude_orders_with_revalidation(client):
    payload = client.post("/api/simulations", json={}).json()
    first, second = payload["orders"][:2]
    response = client.patch(
        f"/api/order-proposals/{payload['simulation_id']}",
        json={
            "adjustments": [
                {
                    "order_id": first["order_id"],
                    "volume_mw": 12.3,
                    "limit_price_eur_mwh": 44.4,
                    "comment": "Trader liquidity view",
                },
...
def test_excessive_trader_volume_fails_physical_validation_and_approval(client):
    payload = client.post("/api/simulations", json={}).json()
    ...
    assert (
        client.post(f"/api/order-proposals/{payload['simulation_id']}/approve").status_code == 409
    )


def test_out_of_range_unavailable_interval_is_rejected(client):
    response = client.post("/api/simulations", json={"battery": {"unavailable_intervals": [999]}})
    assert response.status_code == 422


def test_validate_recomputes_and_marks_orders_validated(client):
    payload = client.post("/api/simulations", json={}).json()
    ...
    assert (
        validated["proposal"]["proposal_terminal_soc_mwh"]
        >= validated["battery"]["target_soc_mwh"] - 0.15
    )


def test_invalid_proposal_cannot_be_exported(client):
    payload = client.post("/api/simulations", json={}).json()
    first = payload["orders"][0]
    client.patch(
        f"/api/order-proposals/{payload['simulation_id']}",
            ...
            "adjustments": [
                {
                    "order_id": first["order_id"],
                    "volume_mw": 1000,
                    "comment": "Deliberate invalid test",
                }
            ]
        },
    )
...
def test_post_export_records_event_but_legacy_get_is_safe(client):
    payload = client.post("/api/simulations", json={}).json()
    simulation_id = payload["simulation_id"]
    assert client.post(f"/api/order-proposals/{simulation_id}/approve").status_code == 200
    before = len(
        [e for e in client.get("/api/audit").json()["items"] if e["simulation_id"] == simulation_id]
    )
...
def test_trader_edit_requires_reason_and_real_change(client):
    payload = client.post("/api/simulations", json={}).json()
    first = payload["orders"][0]
    assert (
        client.patch(
    ...
    assert (
        client.patch(
            f"/api/order-proposals/{payload['simulation_id']}",
            json={"adjustments": [{"order_id": first["order_id"], "comment": "No actual change"}]},
        ).status_code
        == 422
    )
    assert (
        client.patch(
            ...
            json={
                "adjustments": [
                    {
                        "order_id": first["order_id"],
                        "volume_mw": first["volume_mw"],
                        "limit_price_eur_mwh": first["limit_price_eur_mwh"],
                        "comment": "Same submitted values",
                    }
                ]
            },
        ).status_code
...
def test_export_requires_current_proposal_approval_and_edit_revokes_it(client):
    payload = client.post("/api/simulations", json={}).json()
    simulation_id = payload["simulation_id"]
    ...
    first = payload["orders"][0]
    edited = client.patch(
        f"/api/order-proposals/{simulation_id}",
        json={
            "adjustments": [
                {
                    "order_id": first["order_id"],
                    "volume_mw": first["volume_mw"] / 2,
                    "comment": "Reduce market exposure",
                }
            ]
        },
    ).json()
    ...
    assert (
        abs(
            edited["proposal"]["implied_dispatch"][-1]["soc_mwh"]
            - edited["proposal"]["proposal_terminal_soc_mwh"]
        )
        < 0.001
    )
    assert client.post(f"/api/order-proposals/{simulation_id}/exports").status_code == 409
...
def test_initial_result_versions_and_proposal_metrics_are_present(client):
    payload = client.post("/api/simulations", json={}).json()
    assert payload["audit"]["schema_version"] == 5
    assert (
        payload["audit"]["assumption_sources"]["market.exchange_fee"]
        == "Excluded; IWB confirmation required"
    )
    ...
    assert (
        payload["summary"]["baseline_proposal_contribution_eur"]
        == payload["summary"]["proposal_contribution_eur"]
    )


def test_manual_forecast_is_used_and_preserved_with_provenance(client):
...
def test_manual_forecast_must_match_the_product_interval_count(client):
    response = client.post("/api/simulations", json={"price_values": [20.0] * 23})
    assert response.status_code == 422


def test_orders_expose_break_even_evidence_and_result_has_sensitivities(client):
    payload = client.post("/api/simulations", json={}).json()
...
def test_sensitivity_cases_reconcile_to_full_optimizer_results(client):
    payload = client.post("/api/simulations", json={}).json()
...
def test_multi_day_policy_records_continuation_assumption(client):
    payload = client.post(
        "/api/simulations",
        json={
...
def test_saved_run_catalogue_is_compact_and_filterable(client):
    hourly = client.post(
        "/api/simulations",
    ...
    summary = next(
        item for item in items if item["simulation_id"] == quarter_hourly["simulation_id"]
    )
    ...
    assert (
        ...
        == quarter_hourly["summary"]["expected_contribution_eur"]
    )
    assert (
        summary["downside_contribution_eur"] == quarter_hourly["risk"]["downside_contribution_eur"]
    )
...
def test_saved_run_catalogue_rejects_invalid_filters(client):
    ...
    assert (
        client.get("/api/simulation-runs", params={"validation_status": "unknown"}).status_code
        == 422
    )
