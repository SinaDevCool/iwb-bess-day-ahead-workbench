...
CASES = [
    ...
    (
        ...
        {},
        {},
        {"scenario_name": "Peak compression", "peak_reduction_eur_mwh": 25},
    ),
    ...
    (
        ...
        {},
        {
            "exchange_fee_eur_per_mwh": 5,
            "exchange_fee_policy": "configured",
            "clearing_fee_eur_per_mwh": 1,
        },
        {},
    ),
    (
        ...
        {},
        {},
        {"horizon_policy": "terminal_value", "terminal_value_eur_per_mwh": 100},
    ),
...
@pytest.mark.parametrize(
    "name,battery_values,market_values,request_values", CASES, ids=[case[0] for case in CASES]
)
def test_completed_run_is_executable_and_reconciled(
    name, battery_values, market_values, request_values
):
    ...
    assert all(
        ...
        <= min(
            ...
            result.battery.max_charge_power_mw
            if row.power_mw < 0
            else result.battery.max_discharge_power_mw,
        )
        + 1e-6
        for row in proposal.implied_dispatch
    )
    assert all(
        abs(order.energy_mwh - order.volume_mw * duration_hours) <= 1e-4 for order in result.orders
    )
    assert (
