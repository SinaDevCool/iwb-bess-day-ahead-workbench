...
def request(orders=(), shift=0):
    ...
    return OrderSimulationRequest(
        prices=[
            p.model_copy(update={"timestamp_utc": p.timestamp_utc + timedelta(minutes=shift)})
            for p in points
        ],
        orders=[
            SubmittedOrder(client_order_id=str(i), delivery_start_utc=points[5].timestamp_utc, **o)
...
def test_naive_forecast_is_validation_error():
    ...
    with pytest.raises(ValueError, match="timezone"):
        OrderSimulationRequest(
            prices=[
                p.model_copy(update={"timestamp_utc": p.timestamp_utc.replace(tzinfo=None)})
                for p in points
            ]
        )
...
def test_batch_evidence_is_explicit():
    ...
    assert sum(x.soc_delta_mwh for x in result.order_results) == pytest.approx(
        result.dispatch[5].soc_mwh - 50, abs=2e-6
    )
...
def test_infeasible_buy_preserves_price_evidence():
    result = run_order_simulation(
        request([dict(side="BUY", order_type="LIMIT", volume_mw=80, limit_price_eur_mwh=40)])
    )
...
def test_each_order_has_one_outcome():
    result = run_order_simulation(
        request(
            [
                dict(side="BUY", order_type="LIMIT", volume_mw=5, limit_price_eur_mwh=1),
                dict(side="BUY", order_type="MARKET", volume_mw=5),
            ]
        )
    )
    assert {o.client_order_id for o in result.submitted_orders} == {
        o.submitted_order.client_order_id for o in result.order_results
    }


def test_economic_components_reconcile():
    result = run_order_simulation(
        request([dict(side="BUY", order_type="MARKET", volume_mw=5.1)] * 3)
    )
    s = result.summary
    assert s.net_contribution_eur == round(
        s.sales_revenue_eur - s.purchase_cost_eur - s.degradation_cost_eur - s.transaction_fee_eur,
        2,
    )
    row = result.dispatch[5]
    assert row.interval_pnl_eur == round(
...
def test_run_type_filter_applies_before_limit():
    from backend.db.repository import list_simulations, save_simulation_with_event

    for index in range(36):
        payload = {
            "simulation_id": str(index),
            "created_at_utc": f"2026-09-09T00:00:{index:02d}Z",
            "run_type": "OPTIMIZATION" if index < 2 else "ORDER_SIMULATION",
        }
...
def test_proposal_adapter_roundtrip_and_type_guards():
    ...
    result = client.post(
        ...
        json={
            "delivery_date": p["proposal"]["delivery_date"],
            "battery": p["proposal"]["battery"],
            "market": p["proposal"]["market"],
            "price_values": [x["price_eur_mwh"] for x in p["proposal"]["forecast_points"]],
            "orders": p["orders"],
            "source_proposal_id": p["proposal"]["simulation_id"],
        },
    )
    ...
    assert r["summary"]["net_contribution_eur"] == pytest.approx(
        p["proposal"]["summary"]["expected_contribution_eur"], abs=0.1
    )
    ...
    assert (
        next(x for x in history if x["simulation_id"] == p["proposal"]["simulation_id"])[
            "display_name"
        ]
        == p["proposal"]["display_name"]
    )
    ...
    assert {
        x["simulation_id"] for x in client.get("/api/workspace-history").json()["items"]
    } == before_ids
