...
from backend.db.repository import add_audit_event, get_simulation, initialize, list_audit_events, list_simulations, save_simulation
...
def _revalidate_payload(payload: dict):
    ...
    payload["summary"].update({key: value for key, value in proposal.items() if key != "implied_soc_mwh"})
...
def forecast(delivery_date: str = "2026-09-09", product_minutes: int = 60):
    market = DEFAULT_MARKET.model_copy(update={"product_minutes": product_minutes})
...
def validate_proposal(simulation_id: str):
    ...
    add_audit_event(simulation_id, datetime.now(timezone.utc).isoformat(), "ORDER_PROPOSAL_VALIDATED", {"status": validation.status, "proposal_contribution_eur": proposal["proposal_contribution_eur"]})
    return payload
...
def edit_proposal(simulation_id: str, edit: OrderProposalEdit):
    ...
    for order in payload["orders"]:
        ...
        eta = math.sqrt(payload["battery"]["round_trip_efficiency"])
    ...
    save_simulation(payload)
    add_audit_event(simulation_id, datetime.now(timezone.utc).isoformat(), "ORDER_PROPOSAL_EDITED", {"changes": changed, "validation_status": validation.status, "proposal": proposal})
    return payload
...
def approve_proposal(simulation_id: str):
    ...
    save_simulation(payload)
    add_audit_event(simulation_id, datetime.now(timezone.utc).isoformat(), "ORDER_PROPOSAL_APPROVED", {"mode": "demo_only", "submitted": False})
    return {"simulation_id": simulation_id, "approval_status": payload["approval_status"], "submitted": False}


@app.get("/api/order-proposals/{simulation_id}/export")
def export_proposal(simulation_id: str):
    payload = get_simulation(simulation_id)
    if payload is None:
        raise HTTPException(status_code=404, detail="Simulation not found")
    if payload["validation"]["status"] != "passed":
