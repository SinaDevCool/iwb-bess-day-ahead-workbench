...
def build_executable_orders(simulation_id, dispatch, market, battery):
    ...
    while validation.status == "failed" and repaired < 10000 and orders:
        ...
        boundary_finding = next(
            (finding for finding in validation.findings if finding.code in {"proposal_soc_below_min", "proposal_soc_above_max"}),
            None,
        )
        ...
        if boundary_finding and boundary_finding.interval is not None:
            ...
            causal = [o for o in candidates if interval_by_start[o.delivery_start_utc] <= boundary_finding.interval]
            ...
            order = min(candidates, key=lambda o: abs(o.expected_contribution_eur / max(o.energy_mwh, 1e-9)))
