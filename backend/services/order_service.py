...
def build_orders(simulation_id: str, rows: list[DispatchRow], market: MarketConfig, battery: BatteryConfig):
    ...
    for row in rows:
        ...
        economics = calculate_interval(side, volume, market.product_minutes / 60, row.price_eur_mwh, battery)
        orders.append(Order(
            ...
            expected_contribution_eur=round(economics.contribution_eur, 2),
            sales_revenue_eur=round(economics.sales_revenue_eur, 2),
            purchase_cost_eur=round(economics.purchase_cost_eur, 2),
            degradation_cost_eur=round(economics.degradation_cost_eur, 2),
            confidence="medium",
