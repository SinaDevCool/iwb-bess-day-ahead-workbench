...
def build_orders(
    simulation_id: str, rows: list[DispatchRow], market: MarketConfig, battery: BatteryConfig
):
    orders = []
    ...
    for row in rows:
        ...
        if side == "BUY":
            ...
            break_even = (
                future_peak * eta2
                - fee * (1 + eta2)
                - 2 * battery.degradation_cost_eur_per_mwh * eta
            )
        ...
        economics = calculate_interval(
            side, volume, market.product_minutes / 60, row.price_eur_mwh, battery, transaction_fee
        )
        ...
        orders.append(
            Order(
                ...
                confidence="medium",
                explanation=(
                    "Charge below the efficiency- and cost-adjusted future sale value"
                    if side == "BUY"
                    else "Discharge above the efficiency- and cost-adjusted replacement cost"
                ),
            )
        )
    return orders
