...
export function validateBattery(battery: Battery): FieldIssues {
  ...
  if (battery.capacity_mwh <= 0)
    issues.capacity_mwh = "Energy capacity must be positive.";
  if (battery.min_soc_mwh < 0)
    issues.min_soc_mwh = "Minimum SoC cannot be negative.";
  ...
  if (
    ...
    issues.initial_soc_mwh =
      "Initial SoC must be inside the configured SoC window.";
  if (
    battery.target_soc_mwh < battery.min_soc_mwh ||
    battery.target_soc_mwh > battery.max_soc_mwh
  )
    issues.target_soc_mwh =
      "End reserve must be inside the configured SoC window.";
  ...
  if (battery.grid_limit_mw <= 0)
    issues.grid_limit_mw = "Grid limit must be positive.";
  if (battery.round_trip_efficiency <= 0 || battery.round_trip_efficiency > 1)
    issues.round_trip_efficiency =
      "Efficiency must be greater than 0% and at most 100%.";
  ...
  if (battery.degradation_cost_eur_per_mwh < 0)
    issues.degradation_cost_eur_per_mwh =
      "Degradation cost cannot be negative.";
  return issues;
}
  ...
  for (const order of orders) {
    ...
    if (!Number.isFinite(volume) || volume <= 0)
      ...
      issues[`${prefix}.volume`] =
        `Use ${market.volume_increment_mw} MW increments.`;
    if (
      !Number.isInteger(order.interval) ||
      order.interval < 0 ||
      order.interval >= intervalCount
    )
    ...
    if (order.orderType === "LIMIT") {
      ...
      if (order.limit.trim() === "" || !Number.isFinite(limit))
        ...
        limit < market.min_price_eur_mwh ||
        limit > market.max_price_eur_mwh
      )
        issues[`${prefix}.limit`] =
        ...
        issues[`${prefix}.limit`] =
          `Use ${market.price_increment_eur_mwh} €/MWh increments.`;
    }
  }
